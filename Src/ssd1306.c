...
//static uint8_t SSD1306_Buffer[SSD1306_WIDTH * SSD1306_HEIGHT / 8 ];//, *SSD1306_Buffer = SSD1306_Buffer_all+1;
...
uint8_t SSD1306_Init(void) {
	...
	//if (HAL_I2C_IsDeviceReady(&heval_I2c, SSD1306_I2C_ADDR, 1, 20000) != HAL_OK) {
		/* Return false */
	//	return 0;
	//}
	...
	SSD1306_WRITECOMMAND(0xCF);//(0xFF);
	...
	SSD1306_WRITECOMMAND(0xF0);//(0xF0); //--set divide ratio
	SSD1306_WRITECOMMAND(0xD9); //--set pre-charge period
	SSD1306_WRITECOMMAND(0x22); //(0x22); //
	//SSD1306_WRITECOMMAND(0x2E); //(0x22); // деактиваци€ скролинга
	...
	// альтернативна€ инициализацич€ диспле€
        // display OFF
...
//    SSD1306_WRITECOMMAND( 0xAE);
// 
//    SSD1306_WRITECOMMAND( 0x00 | 0x0);            // low col = 0
// 
//    SSD1306_WRITECOMMAND( 0x10 | 0x0);           // hi col = 0
//    SSD1306_WRITECOMMAND( 0x40 | 0x0);            // line #0
// 
//    SSD1306_WRITECOMMAND( 0x81);                   // Set Contrast 0x81
//    SSD1306_WRITECOMMAND( 0xCF);
//    SSD1306_WRITECOMMAND( 0xA1);                    // Segremap - 0xA1
//    SSD1306_WRITECOMMAND( 0xC8);                    // COMSCAN DEC 0xC8 C0
//    SSD1306_WRITECOMMAND( 0xA6);                    // Normal Display 0xA6 (Invert A7)
// 
//    SSD1306_WRITECOMMAND( 0xA4);                // DISPLAY ALL ON RESUME - 0xA4
//    SSD1306_WRITECOMMAND( 0xA8);                    // Set Multiplex 0xA8
//    SSD1306_WRITECOMMAND( 0x3F);                    // 1/64 Duty Cycle 
// 
//    SSD1306_WRITECOMMAND( 0xD3);                    // Set Display Offset 0xD3
//    SSD1306_WRITECOMMAND( 0x0);                     // no offset
// 
//    SSD1306_WRITECOMMAND( 0xD5);                    // Set Display Clk Div 0xD5
//    SSD1306_WRITECOMMAND( 0x80);                    // Recommneded resistor ratio 0x80
// 
//    SSD1306_WRITECOMMAND( 0xD9);                  // Set Precharge 0xd9
//    SSD1306_WRITECOMMAND( 0xF1);
// 
//    SSD1306_WRITECOMMAND( 0xDA);                    // Set COM Pins0xDA
//    SSD1306_WRITECOMMAND( 0x12);
// 
//    SSD1306_WRITECOMMAND( 0xDB);                 // Set VCOM Detect - 0xDB
//    SSD1306_WRITECOMMAND( 0x40);
// 
//    SSD1306_WRITECOMMAND( 0x20);                    // Set Memory Addressing Mode
//    SSD1306_WRITECOMMAND( 0x00);                    // 0x00 - Horizontal
// 
//    SSD1306_WRITECOMMAND( 0x40 | 0x0);              // Set start line at line 0 - 0x40 
// 
//    SSD1306_WRITECOMMAND( 0x8D);                    // Charge Pump -0x8D
//    SSD1306_WRITECOMMAND( 0x14);
// 
// 
//    SSD1306_WRITECOMMAND( 0xA4);              //--turn on all pixels - A5. Regular mode A4
//    SSD1306_WRITECOMMAND( 0xAF);            
...
void SSD1306_UpdateScreen(void) {
	...
	for (m = 0; m < 8; m++) {
		...
		/* Write multi data */
		ssd1306_I2C_WriteMulti( SSD1306_I2C_ADDR, 0x40, &SSD1306_Buffer_all[SSD1306_WIDTH * m], SSD1306_WIDTH);
                //HAL_Delay(3);
	}
}
...
//	HAL_I2C_Master_Transmit_DMA(&heval_I2c, SSD1306_I2C_ADDR, SSD1306_Buffer_all, SSD1306_WIDTH * SSD1306_HEIGHT / 8 + 1);
...
void ssd1306_I2C_WriteMulti(uint8_t address, uint8_t reg, uint8_t* data, uint16_t count) {
  ...
  data[0]= reg;
	...
	//for(i = 1; i <= count; i++)
	//	dt[i] = data[i-1];
	HAL_I2C_Master_Transmit(&hi2c2, address, data, count+1, 1000);
}
